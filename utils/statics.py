...
def evaluator(sparse_pred, sparse_gt, raw_gt):  # , cw, cwq):
    device = sparse_pred.device
    ...
    # Calculate the Rho Cosine Similarity (QCS)
    nt = 32
    nc = 32
    nc_expand = 257
    ...
    sparse_pred = sparse_pred[..., 0] + 1j * sparse_pred[..., 1]
    raw_pred = torch.fft.fft(sparse_pred, dim=-1)[:, :, :125]
    raw_pred = torch.stack((raw_pred.real, raw_pred.imag), dim=3)
    ...
    qcs = (norm_cross / (norm_pred * norm_gt)).mean()
